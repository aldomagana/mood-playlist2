...
app.get('/internal/debug-create/:spotify_id', async (req, res) => {
  ...
  // build an expanded pool of tracks and artist top-tracks to increase variety
  let pool = (recs.tracks || []).map(t => {
    const tt = t.track || t;
    return { id: tt.id, uri: tt.uri, artists: tt.artists || [] };
  }).filter(x => x && x.id);
  try {
    // pick a few unique artists from the pool and fetch their top tracks
    const artistIds = [...new Set(pool.flatMap(p => (p.artists[0] && p.artists[0].id) ? [p.artists[0].id] : []))].slice(0,6);
    for (const aid of shuffleArray(artistIds).slice(0,3)){
      try {
        const top = await spotify.getArtistTopTracks(accessToken, aid, 'US');
        if (top && top.tracks) {
          for (const t of top.tracks) pool.push({ id: t.id, uri: t.uri, artists: t.artists || [] });
        }
      } catch (ea){ /* ignore artist fetch errors */ }
    }
  } catch (e) { /* ignore */ }

  // dedupe tracks and limit to at most 2 tracks per artist
  const seen = new Set();
  const artistCounts = {};
  const finalTracks = [];
  for (const tr of shuffleArray(pool)){
    if (!tr || !tr.id) continue;
    if (seen.has(tr.id)) continue;
    const artistId = (tr.artists[0] && tr.artists[0].id) || 'unknown';
    artistCounts[artistId] = (artistCounts[artistId] || 0) + 1;
    if (artistCounts[artistId] > 2) continue;
    seen.add(tr.id);
    finalTracks.push(tr.uri);
    if (finalTracks.length >= 20) break;
  }
  const trackUris = shuffleArray(finalTracks);
...
app.post('/api/create-playlist', async (req, res) => {
  ...
  try {
    const artistIds = [...new Set(pool.flatMap(p => (p.artists[0] && p.artists[0].id) ? [p.artists[0].id] : []))].slice(0,8);
    for (const aid of shuffleArray(artistIds).slice(0,4)){
      try {
        const top = await spotify.getArtistTopTracks(accessToken, aid, 'US');
        if (top && top.tracks) for (const t of top.tracks) pool.push({ id: t.id, uri: t.uri, artists: t.artists || [] });
      } catch (ea){ /* ignore */ }
    }
  } catch(e){ /* ignore */ }
